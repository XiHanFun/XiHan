...
namespace XiHan.Infrastructure.Persistence.Bases;
...
public class BaseRepository<TEntity> : SimpleClient<TEntity>, IRepository<TEntity> where TEntity : class, new()
{
    ...
    /// <returns></returns>
    public virtual async Task<PageResponseDataDto<TEntity>> QueryPageAsync(PageInfoDto page)
    {
        return await Context.Queryable<TEntity>()
            .ToPageDataDto(page);
    }
    ...
    /// <param name="isOrderAsc">是否正序排序</param>
    /// <returns></returns>
    public virtual async Task<PageResponseDataDto<TEntity>> QueryPageAsync(PageInfoDto page, Expression<Func<TEntity, object>> orderExpression, bool isOrderAsc = true)
    {
        return await Context.Queryable<TEntity>()
            .OrderBy(orderExpression, isOrderAsc ? OrderByType.Asc : OrderByType.Desc)
            .ToPageDataDto(page);
    ...
    /// <param name="page">分页实体</param>
    /// <returns></returns>
    public virtual async Task<PageResponseDataDto<TEntity>> QueryPageAsync(Expression<Func<TEntity, bool>> whereExpression, PageInfoDto page)
    {
        return await Context.Queryable<TEntity>().Where(whereExpression)
            .ToPageDataDto(page);
    }
    ...
    /// <param name="isOrderAsc">是否正序排序</param>
    /// <returns></returns>
    public virtual async Task<PageResponseDataDto<TEntity>> QueryPageAsync(Expression<Func<TEntity, bool>> whereExpression, PageInfoDto page, Expression<Func<TEntity, object>> orderExpression, bool isOrderAsc = true)
    {
        return await Context.Queryable<TEntity>().Where(whereExpression)
            .OrderBy(orderExpression, isOrderAsc ? OrderByType.Asc : OrderByType.Desc)
            .ToPageDataDto(page);
