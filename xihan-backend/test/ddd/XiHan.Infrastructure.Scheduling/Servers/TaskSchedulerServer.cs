...
public class TaskSchedulerServer(IJobFactory jobFactory) : ITaskSchedulerServer
{
    ...
    public async Task<HttpResponseResult> CreateTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            ...
            return HttpResponseResult.Success($"添加计划【{sysJob.Name}】成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> DeleteTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            JobKey jobKey = new(sysJob.BaseId.ToString(), sysJob.Group);
            _ = await _scheduler.DeleteJob(jobKey);
            return HttpResponseResult.Success($"删除计划任务【{sysJob.Name}】成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> ModifyTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            JobKey jobKey = new(sysJob.BaseId.ToString(), sysJob.Group);
            ...
            return HttpResponseResult.Success($"修改计划【{sysJob.Name}】成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> StartTaskScheduleAsync()
    {
        try
        {
            _scheduler.JobFactory = _jobFactory;
            // 计划任务已经开启
            if (_scheduler.IsStarted) return HttpResponseResult.Continue();

            // 等待任务运行完成
            await _scheduler.Start();
            return HttpResponseResult.Success("开启计划任务成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> StopTaskScheduleAsync()
    {
        try
        {
            _scheduler.JobFactory = _jobFactory;
            // 计划任务已经停止
            if (_scheduler.IsShutdown) return HttpResponseResult.Continue();
            // 等待任务运行停止
            await _scheduler.Shutdown();
            return HttpResponseResult.Success($"停止计划任务成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> RunTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            JobKey jobKey = new(sysJob.BaseId.ToString(), sysJob.Group);
            ...
            if (triggers.Count <= 0) return HttpResponseResult.BadRequest($"未找到任务[{jobKey.Name}]的触发器！");

            await _scheduler.TriggerJob(jobKey);
            return HttpResponseResult.Success($"计划任务[{jobKey.Name}]运行成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> PauseTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            JobKey jobKey = new(sysJob.BaseId.ToString(), sysJob.Group);
            ...
            return HttpResponseResult.Success($"暂停计划任务:【{sysJob.Name}】成功！");
        }
        catch (Exception ex)
        {
    ...
    /// <returns></returns>
    /// <exception cref="CustomException"></exception>
    public async Task<HttpResponseResult> ResumeTaskScheduleAsync(SysJob sysJob)
    {
        try
        {
            JobKey jobKey = new(sysJob.BaseId.ToString(), sysJob.Group);
            if (!await _scheduler.CheckExists(jobKey)) return HttpResponseResult.BadRequest($"未找到计划任务【{sysJob.Name}】！");

            await _scheduler.ResumeJob(jobKey);
            return HttpResponseResult.Success($"恢复计划任务【{sysJob.Name}】成功！");
        }
        catch (Exception ex)
        {
