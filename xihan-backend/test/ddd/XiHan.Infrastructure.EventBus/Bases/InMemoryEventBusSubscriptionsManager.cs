...
public class InMemoryEventBusSubscriptionsManager : IEventBusSubscriptionManager
{
    ...
    /// <returns></returns>
    public IEnumerable<SubscriptionModel> GetHandlersForEvent<T>()
        where T : IntegrationEvent
    {
    ...
    private void DoAddSubscription(Type handlerType, string eventName, bool isDynamic)
    {
        ...
        if (isDynamic)
        {
            _handlers[eventName].Add(SubscriptionModel.SubscriptDynamic(handlerType));
        }
        else
        {
            _handlers[eventName].Add(SubscriptionModel.Subscript(handlerType));
        }
    }
    ...
    /// <param name="eventName">事件名称</param>
    /// <returns></returns>
    private SubscriptionModel? FindDynamicSubscriptionToRemove<TH>(string eventName)
    ...
    private SubscriptionModel? DoFindSubscriptionToRemove(string eventName, Type handlerType)
    {
        if (!HasSubscriptionsForEvent(eventName))
        {
            return null;
    ...
    private void DoRemoveHandler(string eventName, SubscriptionModel? subsToRemove)
    {
