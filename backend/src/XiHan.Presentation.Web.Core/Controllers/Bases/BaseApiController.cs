...
// Guid:363f10b9-3b95-459a-91f7-47da16913e0f
// Author:zhaifanhua
// Email:me@zhaifanhua.com
// CreateTime:2024/3/28 5:57:48
// ----------------------------------------------------------------
...
using XiHan.Presentation.Web.Core.Commons.Swagger;

namespace XiHan.Presentation.Web.Core.Controllers.Bases;
...
public class BaseApiController : ControllerBase
{
    ...
    protected async Task<IEnumerable<string>> UploadFile(IEnumerable<IFormFile> files)
    {
        ...
        if (formFiles.Length == 0)
            throw new CustomException("没有上传文件。");
    ...
    protected async Task<IEnumerable<T>> ImportExcel<T>(IFormFile file, string sheetName) where T : class, new()
    {
        if (!Path.GetExtension(file.FileName).Equals(".xlsx", StringComparison.OrdinalIgnoreCase))
            throw new CustomException("无效的文件格式。只支持xlsx文件。");
        // 唯一文件名
        var uniqueFileName = FileHelper.GetUniqueFileName(file.FileName);
        // 导入文件路径
