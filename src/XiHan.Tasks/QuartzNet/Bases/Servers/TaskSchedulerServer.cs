...
public class TaskSchedulerServer : ITaskSchedulerServer
{
    ...
    public async Task<CustomResult> CreateTaskScheduleAsync(SysTasks sysTasks)
    {
        try
        {
            var jobKey = new JobKey(sysTasks.BaseId.ToString(), sysTasks.JobGroup);
            if (await _scheduler.CheckExists(jobKey)) throw new Exception($"该计划任务已经在执行【{sysTasks.JobName}】,请勿重复添加！");
            ...
            return CustomResult.Success($"添加计划【{sysTasks.JobName}】成功！");
        }
        catch (Exception ex)
        {
            throw new CustomException($"添加计划任务【{sysTasks.JobName}】失败！", ex);
        }
    }
    ...
    public async Task<CustomResult> DeleteTaskScheduleAsync(SysTasks sysTasks)
    {
        try
        {
            ...
            return CustomResult.Success($"删除计划任务【{sysTasks.JobName}】成功！");
        }
        catch (Exception ex)
        {
            throw new CustomException($"删除计划任务【{sysTasks.JobName}】失败！", ex);
        }
    }
    ...
    public async Task<CustomResult> ModifyTaskScheduleAsync(SysTasks sysTasks)
    {
        try
        {
            ...
            if (await _scheduler.CheckExists(jobKey))
                ...
                await _scheduler.DeleteJob(jobKey);
            return CustomResult.Success($"修改计划【{sysTasks.JobName}】成功！");
        }
        catch (Exception ex)
        {
            throw new CustomException($"修改计划任务【{sysTasks.JobName}】失败！", ex);
        }
    }
    ...
    public async Task<CustomResult> RunTaskScheduleAsync(SysTasks sysTasks)
    {
        ...
        {
            throw new CustomException($"执行计划任务【{sysTasks.JobName}】失败", ex);
        }
    }
    ...
    public async Task<CustomResult> PauseTaskScheduleAsync(SysTasks sysTasks)
    {
        try
        {
            ...
            return CustomResult.Success($"暂停计划任务:【{sysTasks.JobName}】成功！");
        }
        catch (Exception ex)
        {
            throw new CustomException($"暂停计划任务【{sysTasks.JobName}】失败！", ex);
        }
    }
    ...
    public async Task<CustomResult> ResumeTaskScheduleAsync(SysTasks sysTasks)
    {
        try
        {
            var jobKey = new JobKey(sysTasks.BaseId.ToString(), sysTasks.JobGroup);
            if (!await _scheduler.CheckExists(jobKey))
                return CustomResult.BadRequest($"未找到计划任务【{sysTasks.JobName}】！");
            await _scheduler.ResumeJob(jobKey);
            return CustomResult.Success($"恢复计划任务【{sysTasks.JobName}】成功！");
        }
        catch (Exception ex)
        {
            throw new CustomException($"恢复计划任务【{sysTasks.JobName}】失败！", ex);
        }
    }
    ...
    private static IJobDetail CreateAssemblyJobDetail(SysTasks sysTasks)
    {
        ...
        // 传入执行程序集
        IJobDetail job = new JobDetailImpl(sysTasks.JobName, sysTasks.JobGroup, jobType);
    ...
    private static IJobDetail CreateNetworkRequestJobDetail(SysTasks sysTasks)
    {
        ...
        // 传入执行程序集
        IJobDetail job = new JobDetailImpl(sysTasks.BaseId.ToString(), sysTasks.JobGroup, jobType);
        return job;
    }
    ...
    private static ITrigger CreateIntervalTrigger(SysTasks sysTasks)
    {
        ...
        if (sysTasks.CycleRunTimes != 0 && sysTasks.CycleHasRunTimes >= sysTasks.CycleRunTimes)
            throw new Exception($"该任务计划已完成:【{sysTasks.JobName}】,无需重复启动,如需启动请修改已循环次数再提交");

        // 触发作业立即运行，然后每N秒重复一次，无限循环
        if (sysTasks.RunTimes > 0)
        {
            var trigger = TriggerBuilder.Create()
                .WithIdentity(sysTasks.BaseId.ToString(), sysTasks.JobGroup)
                .StartAt(starRunTime)
                .EndAt(endRunTime)
                .WithSimpleSchedule(x => x.WithIntervalInSeconds(sysTasks.IntervalSecond)
                    .WithRepeatCount(sysTasks.RunTimes))
                .ForJob(sysTasks.BaseId.ToString(), sysTasks.JobGroup)
                .Build();
            return trigger;
        }
        else
